...
- 每天 10:00 UTC（台北时间 18:00）后检查仓库当天发布的数据，再同步本国热词、当前城市配置和 IP 质量探针；User-Agent 池每 30 天更新一次。仓库尚未发布或热词、城市配置同步失败时，每小时重试，当天成功后停止重复同步。
...
- GitHub Actions 每天 03:00 UTC 定时触发，从 Google Trends RSS 抓取各国家或地区的热门词，合并旧记录后每个地区最多保留 100 条。实际启动和完成时间可能延迟。
...
定时任务调用 `bash /opt/ip_sentinel/core/updater.sh --scheduled`，检查 `data/keywords/updated_at` 中的 UTC 发布日期。该文件由每日工作流与热词、城市配置一同提交；日期不是当天或读取失败时，Agent 保留旧数据并等待重试。首次启用这套调度前，每日工作流需要成功运行一次以生成发布标记。手动运行 `bash /opt/ip_sentinel/core/updater.sh` 会立即尝试更新，不受时间和发布日期限制。
...
- 所有 Shell 脚本的语法检查；
- Master 安全策略测试；
- Telegram 面板编辑、聊天隔离、重复点击和编辑失败后的补发测试；
- 注册重发测试；
- 每日更新器的下载失败、坏数据和保留旧文件测试；
- UTC 同步时段、发布日期、失败重试、当天去重，以及两套安装入口的 systemd、Cron、Alpine 调度生成测试；
- 地图、城市配置和热词的数据契约测试；
- 热词全部抓取失败时的退出状态测试。
